import { AutoGeneratedSignatures, FlowEnvironment, FlowGraph, FlowNode, FlowSignature, FlowSignatureId, MapTypeSpecifier, autoGeneratedPrefix, createAutoGeneratedId } from "../types";
...
import { memoObjectByFlatEntries, memFreeze } from "../utils/functional";
...
export const validateFlowGraph = memFreeze((
    ...
    for (let i = 0; i < nodeEntries.length; i++) {
        ...
        if (node.signature === createAutoGeneratedId(AutoGeneratedSignatures.Output)) {
            outputIndex = i;
            break;
        }
    }
...
export const collectFlowDependencies = memFreeze((flow: FlowGraph) => {
    ...
    for (const node of Object.values(flow.nodes) as FlowNode[]) {
        const isAutoGenerated = node.signature.startsWith(autoGeneratedPrefix);
        if (!isAutoGenerated) {
