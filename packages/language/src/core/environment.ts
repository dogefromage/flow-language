...
import { AutoGeneratedSignatures, FlowSignature, InputRowSignature, OutputRowSignature, createAutoGeneratedId } from "../types/signatures";
...
const generateTypeSyntaxSignatures = memoizeMulti((name: string, spec: TypeSpecifier) => {
    if (spec == null ||
        typeof spec !== 'object' ||
        spec.type === 'list'
    ) {
    ...
    const combiner: FlowSignature = {
        id: createAutoGeneratedId(AutoGeneratedSignatures.Combine, name),
        name: `Combine ${name}`,
    ...
    const separator: FlowSignature = {
        id: createAutoGeneratedId(AutoGeneratedSignatures.Separate, name),
        name: `Separate ${name}`,
